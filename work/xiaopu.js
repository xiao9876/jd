...
!(async () => {
    ...
    for (let i = 0; i < iboxpayheaderArr.length; i++) {
        if (iboxpayheaderArr[i]) {
            ...
            iboxpayheadertime = JSON.parse(iboxpayheaderArr[i]).traceid.slice(20, 33);
        ...
        if ($.isRequest) {
            if ($request.url.indexOf("give_gold_coin_by_video") !== -1 && $request.body.indexOf("isFinishWatch") !== -1 && $request.body.indexOf('"type":1') >= 0) {
                ...
                if (iboxpaybody) {
                    ...
                    const body = JSON.parse(iboxpaybody);
                    body.type = 2;
                    const goldbody = JSON.stringify(body);
                    $.setdata(goldbody, `goldbody${$.idx}`)
            ...
            if ($.isResponse && $request.url.indexOf("login_by_wx") > 0) {
                ...
                if (body) {
                    ...
                    if (result.resultCode === 1) {
                        ...
                        $.setdata(result.data.refreshToken, "iboxrefreshtoken");
...
function iboxpay() {
    return new Promise((resolve) => {
        ...
        headers.traceid = headers.traceid.replace(iboxpayheadertime, Date.now())
        let Url = {
    ...
    return new Promise((resolve) => {
        ...
        $.post(Url, async (err, resp, data) => {
            try {
                ...
                resolve(data.resultCode == 1 && $.complete.data.goldCoinNumber > 0)
            } catch (e) {
                $.logErr(e, resp);
            } finally {
                resolve()
...
async function Msg() {
    ...
    const hour = new Date().getHours();
    const minutes = new Date().getMinutes();
    if ($.isNode() && hour === 21 && minutes > 30 && minutes < 32) {
